...
def contactus():
    ...
    if request.method == "POST":
        try:
            ...
            b = request.form["q10_contactNumber"]
            c = request.form["q7_email"]
            d = request.form["q4_message4"]
            with sqlite3.connect("projectDB.db") as con:
                cur = con.cursor()
                cur.execute("insert into queries (name,phone,mail,message) values (?,?,?,?)", (a, b, c, d))
                con.commit()
...
def viewQueries():
    ...
    rows = c.fetchall()
    return render_template("viewQueries.html", rows=rows)
...
def updateProDetails():
    ...
    db.execute("update products set name=:name, onSalePrice=:price, kind=:category where id=:pid",name=name, price=price, category=category, pid=pid)
    shoppingCart = []
    shopLen = len(shoppingCart)
    totItems, total, display = 0, 0, 0
    # Retrieve all shirts ever bought by current user
